...
if __name__ == "__main__":
    ...
    parser.add_argument('-min', '--minimalSilence', type=int,
                        default=500, help='minimal silence length in ms')
    parser.add_argument('-start', '--startPadding', type=int, default=100,
                        help='amount of time that is not cut from the beginning of silence')
    parser.add_argument('-end', '--endPadding', type=int, default=100,
                        help='amount of time that is not cut from the end of silence')
    parser.add_argument('-t', '--threshold', type=float, default=0.0001,
                        help='maximal noise value to be considered as sound')
    parser.add_argument('-v', '--verbose', action='store_true',
                        help='prints which sections are cut off instead of progress bar')
    parser.add_argument('--threads', type=int, default=1,
                        help='how many threads to use for final video encoding')
    parser.add_argument('-crf', type=int, default=17,
                        help='crf of ffmpeg h264 encoding')
    ...
    tresholdSamples = list(
        map(lambda s: args.threshold > max(s) > -args.threshold, audio_samples))
    ...
    while True:
        if not args.verbose:
            print(' '*20, end='\r')
            print('Working:', round(lastEndIndex /
                  samplesLength * 100, 2), '%', end='\r')
        if args.verbose:
            print(f'Video time to analyze: {getTimeStr(video.duration*1000)}')

        indices, found = findFirstSilence(
            tresholdSamples, minimalSamplesInRow, startSearchAt=lastEndIndex)
        if found:
            startMs = indices[0]//(SAMPLE_RATE//1000) + args.startPadding
            endMs = indices[1]//(SAMPLE_RATE//1000) - args.endPadding
            beforeSilence = video.subclip(t_start=getTimeStr(
                lastSilenceEndMs), t_end=getTimeStr(startMs))
            clipsWithSound.append(beforeSilence)
            # beforeSilence = video.subclip(t_end=getTimeStr(startMs))
            if args.verbose:
                print(f'Cutting out:', getTimeStr(
                    lastSilenceEndMs), '-', getTimeStr(startMs))
            ...
            clipsWithSound.append(video.subclip(t_start=getTimeStr(lastSilenceEndMs)))
            break
